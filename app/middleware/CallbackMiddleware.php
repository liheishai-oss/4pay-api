...
class CallbackMiddleware implements MiddlewareInterface
{
    ...
    {
        // 记录调试信息
        \support\Log::info('CallbackMiddleware 开始处理', [
            'uri' => $request->uri(),
        ...
        // 记录调试信息
        \support\Log::info('CallbackMiddleware 供应商查找', [
    ...
    private function validateCallbackIpWhitelist(Supplier $supplier, string $clientIp): void
    {
        ...
        if (empty($supplier->callback_whitelist_ips)) {
            \support\Log::info('CallbackMiddleware IP白名单为空，允许所有IP', [
        ...
        foreach ($whitelistIps as $allowedIp) {
            if ($this->ipMatches($clientIp, $allowedIp)) {
                \support\Log::info('CallbackMiddleware IP验证通过', [
                    'supplier_id' => $supplier->id,
                    'client_ip' => $clientIp,
        ...
        // IP不在白名单中，抛出异常
        \support\Log::warning('CallbackMiddleware IP验证失败', [
            'supplier_id' => $supplier->id,
            'client_ip' => $clientIp,
