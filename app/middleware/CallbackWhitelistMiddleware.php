 ...
 * 专门处理支付服务商回调的IP白名单验证
 */
class CallbackWhitelistMiddleware implements MiddlewareInterface
{
    /**
     * 处理回调IP白名单验证
     * 
    ...
    {
        // 记录调试信息
        \support\Log::info('CallbackWhitelistMiddleware 开始处理', [
            'uri' => $request->uri(),
        ...
        $uri = $request->uri();
        $paymentName = '';
        if (preg_match('/\/callback\/([^\/\?]+)/', $uri, $matches)) {
            $paymentName = $matches[1];
        }
        ...
        // 记录调试信息
        \support\Log::info('CallbackWhitelistMiddleware 供应商查找', [
        ...
        // 验证IP白名单（从支付通道获取）
        $this->validateCallbackIpWhitelist($supplier, $paymentName, $clientIp);
     ...
     * @throws MyBusinessException
     */
    private function validateCallbackIpWhitelist(Supplier $supplier, string $paymentName, string $clientIp): void
    {
        // 从支付通道获取IP白名单
        $whitelistIps = $this->getCallbackIpsFromChannels($supplier->id);
        
        // 如果没有设置IP白名单，则允许所有IP
        if (empty($whitelistIps)) {
            \support\Log::info('CallbackWhitelistMiddleware 支付通道IP白名单为空，允许所有IP', [
        ...
        foreach ($whitelistIps as $allowedIp) {
            if ($this->ipMatches($clientIp, $allowedIp)) {
                \support\Log::info('CallbackWhitelistMiddleware IP验证通过', [
                    'supplier_id' => $supplier->id,
                    'payment_name' => $paymentName,
                    'client_ip' => $clientIp,
        ...
        // IP不在白名单中，抛出异常
        \support\Log::warning('CallbackWhitelistMiddleware IP验证失败', [
            'supplier_id' => $supplier->id,
            'payment_name' => $paymentName,
            'client_ip' => $clientIp,
            'whitelist_ips' => $whitelistIps
        ]);
        throw new MyBusinessException("回调IP地址 {$clientIp} 不在允许的访问列表中", 403);
    }
     ...
     * @return array
     */
    private function getCallbackIpsFromChannels(int $supplierId): array
    {
        // 查询该供应商下的所有支付通道
        $channels = \app\model\PaymentChannel::where('supplier_id', $supplierId)
            ->where('status', 1) // 只查询启用的通道
            ->get();
        
        $allCallbackIps = [];
        
        foreach ($channels as $channel) {
            // 从通道的basic_params中获取callback_ips
            if (!empty($channel->basic_params) && is_array($channel->basic_params)) {
                $callbackIps = $channel->basic_params['callback_ips'] ?? '';
                
                if (!empty($callbackIps)) {
                    if (is_string($callbackIps)) {
                        // 支持逗号分隔的字符串格式
                        $ips = array_filter(array_map('trim', explode(',', $callbackIps)));
                        $allCallbackIps = array_merge($allCallbackIps, $ips);
                    } elseif (is_array($callbackIps)) {
                        // 支持数组格式
                        $allCallbackIps = array_merge($allCallbackIps, $callbackIps);
                    }
                }
            }
        }
        ...
        // 去重并过滤空值
        return array_unique(array_filter($allCallbackIps));
    }
